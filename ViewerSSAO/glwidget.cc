...
const char kBlurVertexShaderFile[] = "../shaders/blur.vert";
...
void GLWidget::initializeGL() {
  ...
  res = LoadProgram(kBlurVertexShaderFile, kBlurFragmentShaderFile,
                         blur_program_.get());
...
void GLWidget::paintGL() {
  ...
  if (initialized_) {
    ...
    if (mesh_ != nullptr) {
      ...
      view_location = ssao_program_->uniformLocation("view");
      model_location = ssao_program_->uniformLocation("model");
...
void GLWidget::paintQuad()
{
    float quadVertices[] =
    {
         -1.0f,-1.0f,-0.05f,   1.0f, 1.0f,-0.05f,   -1.0f, 1.0f,-0.05f,
          1.0f,-1.0f,-0.05f,   1.0f, 1.0f,-0.05f,   -1.0f,-1.0f,-0.05f
    };
